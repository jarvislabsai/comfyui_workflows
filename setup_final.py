...
def run_command(command, in_venv=False):
    original_dir = os.getcwd()
    if in_venv:
        os.chdir("/home/ai-toolkit")
        activate_cmd = f"source {os.path.join('venv', 'bin', 'activate')} && {command}"
...
# Change to the parent directory of ai-toolkit
os.chdir("/home")
...
setup_commands = "pip install torch && pip install -r requirements.txt"

run_command(setup_commands, in_venv=True)
run_command("pip install flash_attn einops timm")
...
#     run_command(command, in_venv=True)
